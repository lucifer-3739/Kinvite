...
import { 
  Cake, Calendar, MapPin, Sparkles, Clock, Check, X, Gift, 
  Share2, Copy, PartyPopper, Music, Utensils, Heart, ChevronRight
} from "lucide-react";
...
interface Birthday01Props {
  event: EventData;
  initialGuests: GuestData[];
  defaultColorCombo?: string;
}

export default function Birthday01({ event, initialGuests, defaultColorCombo }: Birthday01Props) {
  // Parse style from coverImage if configured
  const getInitialCombo = () => {
    ...
    return "ocean_blue"; // Default vibrant color for birthday
  };
  ...
  const handleCopy = async () => {
    if (typeof window !== "undefined") {
      try {
        ...
        setCopied(true);
        toast.success("Party invitation link copied to clipboard!");
  ...
  const handleRsvpSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!rsvpChoice) {
      toast.error("Please select whether you will attend the party.");
      return;
    }
    if (!rsvpName.trim()) {
    ...
    try {
      const result = await createRsvpAction({
        ...
        relation: rsvpDiet ? `Dietary: ${rsvpDiet}` : "Friend",
        side: "birthday_guest"
      });
      if (result.success) {
        setIsSubmitted(true);
        toast.success(rsvpChoice === "yes" ? "Yay! We can't wait to party with you!" : "Thank you for letting us know.");
      } else {
  ...
  return (
    ...
    >
      {/* 1. Interactive Theme Color Palette Bar */}
      <header 
      ...
      >
        <div className="flex items-center gap-2">
          <PartyPopper className="size-4" style={{ color: combo.c3 }} />
      ...
      <main className="max-w-xl mx-auto px-4 pt-10 pb-6 text-center space-y-8">
        ...
        >
          <Sparkles className="size-3.5" style={{ color: combo.c3 }} />
        ...
        <div className="space-y-3">
          ...
          >
            {event.title || "Celebrate With Us!"}
          </h1>
          <p 
            className="text-sm sm:text-base font-medium max-w-md mx-auto leading-relaxed"
            style={{ color: `${combo.c4}CC` }}
          >
            {event.description || "Join us for an unforgettable evening of music, drinks, laughter, and birthday cake!"}
          </p>
        </div>

        {/* 3. Live Countdown Glassmorphic Card */}
        <div 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
          ...
          >
            ...
            <div>
              ...
              <p className="font-bold text-sm mt-0.5" style={{ color: combo.c4 }}>
                {event.venue || "Private Banquet & Lounge"}
              </p>
              <a 
                href={`https://maps.google.com/?q=${encodeURIComponent(event.venue || "Party Venue")}`}
                target="_blank"
                rel="noreferrer"
        ...
        >
          <div className="flex items-center gap-2">
            <Cake className="size-4" style={{ color: combo.c3 }} />
          ...
          <div className="space-y-3 pt-2">
            {[
              { time: "6:00 PM", title: "Doors Open & Welcome Cocktails", icon: PartyPopper },
              { time: "7:30 PM", title: "Birthday Toast & Cake Cutting Ceremony", icon: Cake },
              { time: "8:30 PM", title: "Buffet Dinner & Special Treats", icon: Utensils },
              { time: "9:30 PM", title: "DJ Music, Dancing & Games", icon: Music }
            ].map((item, idx) => (
              <div 
                key={idx}
        ...
        >
          <div className="text-center space-y-1">
            ...
            <p className="text-xs" style={{ color: `${combo.c4}AA` }}>
              Please let us know by submitting your response below.
            </p>
          </div>
          ...
          {isSubmitted ? (
            ...
            >
              ...
              <p className="text-xs" style={{ color: `${combo.c4}CC` }}>
                Thank you, {rsvpName}! We have saved your confirmation.
              </p>
            </div>
          ) : (
            <form onSubmit={handleRsvpSubmit} className="space-y-4">
              ...
              >
                {isSubmitting ? "Saving RSVP..." : "Confirm My RSVP"}
              </Button>
            </form>
          )}
        </div>

        {/* 7. Gift & Presence Note */}
        <div 
        ...
        >
          <Gift className="size-5 mx-auto" style={{ color: combo.c3 }} />
